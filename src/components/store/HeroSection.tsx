...
export function HeroSection() {
  ...
  return (
    <section id="hero" aria-label="Product Showcase" className="relative pt-1 pb-4 md:py-4 lg:py-6 px-3 sm:px-6 lg:px-8 max-w-7xl mx-auto">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-3 sm:gap-6 lg:gap-8 items-start">
        {/* Left Column: Product Showcase Gallery (Mobile height capped at 34vh for above-the-fold clearance) */}
        <div className="lg:col-span-7 flex flex-col gap-2 sm:gap-3">
          {/* Main Showcase Image (Alibaba Precision Magnifier Zoom Panel) */}
          <div
            className="relative w-full aspect-square max-h-[32vh] sm:max-h-[40vh] lg:max-h-[420px] rounded-2xl sm:rounded-3xl overflow-hidden bg-stone-200/80 border border-stone-300/80 shadow-md sm:shadow-lg group cursor-zoom-in select-none"
          ...
          >
            ...
            <div className="absolute top-2.5 left-2.5 flex flex-wrap gap-1.5 pointer-events-none z-10">
              <span className="glass-dark-card text-[10px] sm:text-[11px] font-medium tracking-wide px-2.5 py-0.5 sm:py-1 rounded-full shadow-md flex items-center gap-1 border border-stone-700/80">
                <Sparkles className="w-3 h-3 text-amber-400" />
                <span>3.0mm Solid Merino Wool</span>
              </span>
              <span className="glass-panel text-stone-900 text-[10px] sm:text-[11px] font-medium tracking-wide px-2.5 py-0.5 sm:py-1 rounded-full shadow-md flex items-center gap-1 border border-stone-200/90 hidden xs:flex">
            ...
            <div className="absolute bottom-2.5 right-2.5 glass-dark-card text-[10px] sm:text-[11px] font-medium px-2.5 py-0.5 rounded-full tracking-wide text-amber-300 border border-stone-700/80 flex items-center gap-1">
              <ZoomIn className="w-3 h-3 text-amber-400" />
              <span>{isZoomed ? "Hover to Pan" : "Micro-Grip Backing"}</span>
            </div>
          </div>

          {/* Gallery Thumbnails (6 WebP Items) */}
          <div className="flex items-center gap-1.5 sm:gap-2 overflow-x-auto py-0.5">
            {images.map((img, idx) => (
              <button
                ...
                className={`relative w-12 h-12 sm:w-18 sm:h-18 lg:w-20 lg:h-20 rounded-lg sm:rounded-xl overflow-hidden border-2 shrink-0 transition-all ${
        ...
        <div className="lg:col-span-5">
          <div className="bg-stone-100/90 backdrop-blur-md border border-stone-300/80 p-3.5 sm:p-5 lg:p-6 rounded-2xl sm:rounded-3xl shadow-lg lg:shadow-xl flex flex-col gap-2 sm:gap-3">
            ...
            <h1 className="font-serif text-xl sm:text-3xl lg:text-4xl font-bold tracking-tight text-stone-900 leading-snug sm:leading-tight">
            ...
            <div className="flex items-center justify-between gap-2 pt-0.5 border-t border-stone-200/80">
              ...
              <span className="text-[10px] sm:text-[11px] font-semibold text-emerald-700 bg-emerald-50 border border-emerald-200 px-2 py-0.5 sm:px-2.5 sm:py-1 rounded-full flex items-center gap-1">
            ...
            <div className="pt-0.5 flex flex-col gap-1.5">
              ...
              <div className="grid grid-cols-3 gap-1 text-center text-[11px] sm:text-xs font-semibold text-stone-800 bg-amber-50/90 border border-amber-200/90 py-1.5 sm:py-2 px-2 rounded-xl shadow-2xs">
                <button
                  type="button"
